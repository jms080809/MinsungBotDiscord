...
TOKEN = "NzgxMzU0NDI2NzY1NDEwMzE0.X78bBA.nGaDJgoeRK1uffRCVM9LLmuOK_4"
...
@bot.command()
async def 임베드(ctx):
    embed = discord.Embed(colour=discord.Colour.green(),title='제목데수네',description = '에엥ㅇ따')
...
async def 프로필(message):
    ...
    embed.add_field(name='가입일',value=str(date.year) + '/' + str(date.month) + '/' + str(date.day),inline=False)
...
async def 행성(message):
    ...
    embed.set_image(url='https://cdn.discordapp.com/avatars/605966157530529802/8f71e4f7a2ff8281a31e633bbaef9d40.png?size=128')
    await message.send(embed=embed)
...
async def 게이(message):
    embed = discord.Embed(description=':Are you gay?(당신은 게이입니까?)',tts=True,color=0x437299,inline=True)
    await message.send(embed = embed)
...
async def 봇(message):
    ...
    embed.set_thumbnail(url='https://cdn.discordapp.com/avatars/781354426765410314/3c6099ede71bf90faf37e2fe2c16823c.png?size=128')
    await message.send(embed=embed)
...
async def 온클(message):
    embed=discord.Embed(colour=0x8b00ff,title='온라인 클래스',description='https://oc11.ebssw.kr/onlineClass/search/onlineClassSearchView.do?schulCcode=05977&schCssTyp=online_poc')
    embed.set_image(url='https://t1.daumcdn.net/cfile/tistory/997A26485E8D08C416')
    await message.send(embed=embed)
    time.sleep(2)
